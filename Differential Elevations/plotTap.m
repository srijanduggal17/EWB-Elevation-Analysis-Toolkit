...
%       This case is for degrees, minutes, seconds input
        for x = 2:rows
%           Get latitude, longitude, and label for current point
            valueLat = raw{x,2};
            valueLong = raw{x,3};
            label = [label; raw{x, 1}];
...
%           Append to vector of latitudes and longitudes
            latIndicies = [latIndicies latValue];
            longIndicies = [longIndicies longValue];

%           Find the index of the nearest latitude and longitude to the
%           inputted values
            [nearestlatind, ~] = dsearchn(latArr,latValue);
            [nearestlongind, ~] = dsearchn(longArr, longValue);
            
%           Find the elevation of that point and append it to a vector
            nearestelevation = sortedElevationData(nearestlatind, nearestlongind);
            elevations = [elevations nearestelevation];
        end
    otherwise
